...
import { configPath, defaultOriginPathMap } from "../config";
import { configFile, indexStyle, pathFile, preview, reactIndexFile, vue3IndexFile } from "../template";
...
export function getPackageJson(type: "cli" | "project" = "cli"): Record<string, any> {
  try {
    const paths = type === "cli" ? path.join(__dirname, "../../package.json") : path.join(process.cwd(), configPath);
...
// 获取icon的config数据
export function getIconConfig(): { filePath: string; fileName: string; content: any } | undefined {
  try {
...
export const getIconFiles = (framework: string): Record<string, any> => {
  return {
    "index.tsx": framework === "react" ? reactIndexFile : vue3IndexFile,
    "index.scss": indexStyle,
    "path.tsx": pathFile,
